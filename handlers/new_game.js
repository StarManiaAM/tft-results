...
async function startRiotHandler(client, channelId) {
    ...
    logger.info(
        `Starting Riot match handler for channel: ${channel.name} (${channelId})`
    );
        ...
        try {
            ...
            for (let user of users) {
                ...
                try {
                    ...
                    if (
                        userErr instanceof RiotApiError &&
                        userErr.isUnauthorized()
                    ) {
                        logger.fatal(
                            `API key is invalid or expired (${userErr.statusCode})`,
                            {
                        ...
                        try {
                            ...
                            logger.error(
                                "Failed to send API key alert to channel",
                                sendErr
                            );
                        }
                        break;
                    ...
                    logger.error(
                        `Failed to process matches for user ${user.username}#${user.tag} (${user.puuid})`,
                        {
                            error: userErr.message,
            ...
            if (errorCount === 0) {
                ...
                consecutiveErrors++;
                logger.warn(
                    `All users failed processing (${consecutiveErrors}/${maxConsecutiveErrors})`
                );
            } else {
            ...
            if (consecutiveErrors >= maxConsecutiveErrors) {
                logger.fatal(
                    `Match processing has failed ${consecutiveErrors} consecutive times. System may be unhealthy.`
                );
                ...
                try {
                    ...
                    logger.error(
                        "Failed to send health alert to channel",
                        sendErr
                    );
                }
            }
    ...
    async function processUserMatch(user, channel, processedPartners) {
        ...
        try {
            ...
            if (error instanceof RiotApiError) {
                if (error.statusCode === 400) {
                    ...
                    logger.warn(
                        `Rate limited while fetching matches for ${user.username}#${user.tag}`
                    );
        ...
        if (cached_data && cached_data.puuid === user.puuid) {
            logger.debug(
                `Match ${last_match} already processed globally for ${user.username}`
            );
        ...
        logger.info(
            `New match detected for ${user.username}#${user.tag}: ${last_match}`
        );
        ...
        if (globalProcessedMatches.has(last_match)) {
            ...
            try {
                ...
                if (error instanceof RiotApiError) {
                    if (error.isNotFound()) {
                        logger.warn(
                            `Match ${last_match} not found for ${user.username}`,
                            {
                                userId: user.puuid,
                                matchId: last_match,
                            }
                        ...
                        logger.warn(
                            `Rate limited while fetching match info ${last_match}`
                        );
                        throw error; // Re-throw to trigger backoff
                    }
                }
        ...
        if (!data) {
            logger.warn(
                `Participant data not found for ${user.username} in match ${last_match}`,
                {
                    userId: user.puuid,
                    matchId: last_match,
        ...
        try {
            ...
            globalProcessedMatches.set(last_match, {
                puuid: user.puuid,
                game_info: game_info,
                timestamp: Date.now(),
            });
        ...
        try {
            ...
            try {
                ...
                if (error instanceof RiotApiError) {
                    if (error.isNotFound()) {
                        logger.warn(
                            `Rank not found for ${user.username}, using UNRANKED`,
                            {
                                userId: user.puuid,
                            }
                        );
            ...
            logger.info(
                `${user.username}#${user.tag} finished #${data.placement}`,
                {
            ...
            logger.error(`Error processing solo match for ${user.username}`, {
                ...
                errorType: err.name,
                statusCode:
                    err instanceof RiotApiError ? err.statusCode : undefined,
                stack: err.stack,
                matchId: last_match,
            });
    ...
    async function processDoubleUpMatch(
        ...
        try {
            ...
            if (teammate) {
                ...
                if (teammateDb) {
                    try {
                        ...
                        try {
                            ...
                            if (
                                ...
                                logger.warn(
                                    `Rank not found for teammate ${teammate.riotIdGameName}`,
                                    {
                        ...
                        const tres = await update_rank_with_delta(
                            teammateDb.puuid,
                            tRankInfo
                        );
                        ...
                        logger.debug(
                            `Processed teammate ${teammateData.username} for ${user.username}`
                        );
                    } catch (teammateErr) {
                        logger.error(
                            `Failed to process teammate ${teammate.riotIdGameName}`,
                            {
                                error: teammateErr.message,
                                errorType: teammateErr.name,
                                statusCode:
                                    teammateErr instanceof RiotApiError
                                        ? teammateErr.statusCode
                                        : undefined,
                                teammateId: teammate.puuid,
                            }
                        );
                    ...
                    teammateData = {
                        ...
                        data: { units: teammate.units },
                        rank: {
                            tier: rankInfo.doubleup?.tier || "UNRANKED",
                            division: rankInfo.doubleup?.division || "",
                            lp: rankInfo.doubleup?.lp || 0,
                        },
                        lpChange: "",
                    };
                }
            ...
            logger.info(
                `${user.username}#${user.tag} - Double Up result #${placement}`,
                {
                    rank: `${current.tier} ${current.division} ${current.lp} LP`,
                    lpChange,
            ...
            logger.error(
                `Error processing double-up match for ${user.username}`,
                {
                    error: err.message,
                    errorType: err.name,
                    statusCode:
        ...
        while (!stopLoop && !isShuttingDown && !apiKeyInvalid) {
            try {
                ...
                if (failureCount > 0) {
                    ...
                    logger.debug(
                        `Using backoff delay: ${nextDelay}ms (failure count: ${failureCount})`
                    );
                }
